...
export function validateTfManifest(manifest: TfManifest): TfValidationMessage[] {
  ...
  if (regions.size > 1) {
    messages.push({
      ...
      detail: `This local generator currently supports one region per Terraform workspace. Found: ${Array.from(regions).join(', ')}.`,
    });
  }
  ...
  manifest.accounts.forEach((account) => {
    if (!account.role_arn) {
      messages.push({
        ...
        detail: 'Local preview can continue, but real deployment will need a cross-account role ARN.',
      });
  ...
  if (!messages.some((message) => message.severity === 'error')) {
    messages.push({
      ...
      detail: 'Manifest is ready for Terraform generation preview. Real deployment still requires plan review and role verification.',
    });
  }
...
export function generateTfReviewSummary(manifest: TfManifest, messages: TfValidationMessage[]): TfReviewSummary {
  ...
  const headline = errors.length
    ...
    : warnings.length
      ...
      : 'Manifest passed local checks and is ready for Terraform plan validation.';
...
# Local preview generated for ${hclString(manifest.deployment_name)}.
# Apply is intentionally disabled until human review and CodeBuild wiring are enabled.
`;
}
...
function recommendationFor(message: TfValidationMessage) {
  ...
  if (title.includes('local validation passed')) return 'Export the files and run terraform fmt, validate, and plan in the controlled runner.';
