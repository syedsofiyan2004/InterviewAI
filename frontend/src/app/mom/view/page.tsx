...
import { AlertCircle, CheckCircle2, Clock, Download, FileText, Loader2, RotateCcw } from 'lucide-react';
...
function MomViewContent() {
  ...
  // Keyed by format rather than a single boolean: asking for the PDF must not grey out the
  // Word button or spin its spinner.
  ...
  const handleDownloadReport = async (format: 'pdf' | 'docx') => {
    ...
    try {
      const { download_url } = await api.getMomReportUrl(id, format);
      window.location.href = download_url;
    } catch (err: any) {
      setError(err.message || `Failed to download the ${format === 'docx' ? 'Word' : 'PDF'} report`);
    } finally {
      setDownloading(null);
    }
  };
  ...
  const handleRetryAnalysis = async () => {
    ...
    setError(null);
    try {
      await api.analyzeMom(id);
      setMom((current) => current ? {
        ...current,
        status: 'PROCESSING',
        error: undefined,
        progress_stage: 'queued',
        progress_message: 'Queued for MOM analysis...',
        analysis_started_at: Date.now(),
      } : current);
      setPollGeneration((generation) => generation + 1);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'The MOM analysis could not be restarted.');
    } finally {
      setRetrying(false);
    }
  };
  ...
  return (
    <div className="space-y-8 pb-8">
          ...
          {result && (
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-lg bg-accent px-4 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90 disabled:opacity-50"
              >
                {downloading === 'pdf' ? <Loader2 size={16} className="animate-spin" /> : <Download size={16} />}
                {downloading === 'pdf' ? 'Preparing PDF' : 'Download PDF'}
              </button>
              {/* Word, because minutes are usually trimmed before they reach a client: this
                  file has real headings and tables, so a section can be cut in one click. */}
              <button
                onClick={() => void handleDownloadReport('docx')}
                disabled={downloading !== null}
                title="Editable Word document, for trimming before you send it"
                className="btn-secondary inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold disabled:opacity-50"
              >
                {downloading === 'docx' ? <Loader2 size={16} className="animate-spin" /> : <FileText size={16} />}
          ...
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <button
                ...
                disabled={retrying}
                className="mt-4 inline-flex items-center justify-center gap-2 rounded-md border border-danger/40 px-3 py-2 text-sm font-semibold text-danger transition-colors hover:bg-danger/10 disabled:cursor-not-allowed disabled:opacity-60"
              >
            ...
            <div className={activeView === 'report' ? 'space-y-6' : 'hidden'} role="tabpanel">
              <section className="card p-6 space-y-4 text-center">
                ...
                <p className="text-sm text-text-secondary max-w-md mx-auto">
                  Download this Minutes of Meeting as a PDF to circulate as-is, or as a Word document
                  to edit down before you send it to a client.
                </p>
                <div className="flex flex-wrap items-center justify-center gap-2">
                  <button
                    onClick={() => void handleDownloadReport('pdf')}
                    disabled={downloading !== null}
                    className="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg bg-accent text-white font-semibold text-sm hover:opacity-90 transition-opacity disabled:opacity-50"
                  >
                    {downloading === 'pdf' ? <Loader2 size={16} className="animate-spin" /> : <Download size={16} />}
                  ...
                  <button
                    onClick={() => void handleDownloadReport('docx')}
                    disabled={downloading !== null}
                    className="btn-secondary inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold disabled:opacity-50"
                  >
                    {downloading === 'docx' ? <Loader2 size={16} className="animate-spin" /> : <FileText size={16} />}
                    {downloading === 'docx' ? 'Preparing Word...' : 'Download Word Report'}
                  </button>
                </div>
              </section>
            </div>
      ...
      {/*
        Only with a COMPLETED result. Applying an edit rewrites this same record in place
        (unlike a calculator revision), so a refetch is all the page needs afterwards.
      */}
      {id && mom?.status === 'COMPLETED' && (
        <ContextChat
          app="mom"
          entityId={id}
          title={result?.title || mom?.title}
          onApplied={() => { void fetchMom(); }}
        />
      )}
    </div>
  );
