...
/**
 * Tabular input is read by shared/workbook.ts, not from here.
 *
 * This file used to carry an extractTableFromBuffer that returned the first sheet
 * with content as a flat grid. That shape cannot express what an uploaded client
 * model actually is -- ten tabs, a header on row 4, rates and assumptions in
 * label/value blocks -- so readWorkbook returns every sheet and
 * api-handler/calculator-workbook.ts interprets them. Keeping both would have left
 * two parsers to fix whenever a sheet surprised us.
 */
